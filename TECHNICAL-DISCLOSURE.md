...
A method for encoding, storing, and advecting arbitrary application-layer metadata in the non-physical degrees of freedom ("ghost modes") of a Lattice Boltzmann fluid simulation using deterministic integer arithmetic. Metadata may be stored within the existing distribution arrays with zero additional memory, and may be advected with the fluid velocity field using a prescribed protocol of ping-pong buffer readback, semi-Lagrangian upstream interpolation, value-dependent readback correction, and forced conservation via external invariant tracking. Under this protocol, metadata is conserved exactly across arbitrary simulation lengths (verified at 0.00% drift over 100+ ticks). The MRT transform cost (for example approximately 0.3ms at 4.7M cells for forward + inverse transform, consisting primarily of additions and subtractions from small integer M matrix entries) may be offset by the elimination of multiple separate advection passes and their associated storage buffers (for example approximately 4-5ms and 142MB saved at 4.7M cells), yielding a net compute reduction of approximately 15x. This technique applies to any LBM lattice and any application requiring per-cell metadata that moves with a simulated fluid, including but not limited to interactive games, scientific visualization, industrial simulation, multi-material flow modeling, or any other context in which metadata must advect with the flow. No prior art is known for using LBM ghost modes as a general-purpose metadata transport mechanism as of the date of this disclosure.
